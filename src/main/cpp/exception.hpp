...
#define CKZG_TRY(result)                                                                                                    \
    {                                                                                                                  \
        ...
        if (___ret != C_KZG_OK) throw KZGException(std::string("C-KZG error: ") + C_KZG_ERRORS[___ret]);                     \
    }

#define BLST_TRY(result)                                                                                                    \
    {                                                                                                                  \
        BLST_ERROR ___ret = (result);                                                                                      \
        if (___ret != BLST_SUCCESS) throw KZGException(std::string("BLST error: ") + BLST_ERRORS[___ret]);                     \
    }
