...
public:
	CKZGSettings(CFFTSettings* fft_settings, std::vector<G1> g1_secrets, std::vector<G2> g2_secrets) {
		...
		new_kzg_settings(&settings, (g1_t*)&g1_secrets[0], (g2_t*)&g2_secrets[0], g1_secrets.size(), &fft_settings->settings);
	}
	~CKZGSettings() {
	...
	G1 compute_proof_single(Poly* poly, Fr x0) {
		G1 ret;
		::compute_proof_single(&ret.g1, &poly->_poly, &x0.fr, &settings);
		return ret;
	}
	G1 compute_proof_poly(Poly* poly, std::vector<Fr> x0) {
		G1 ret;
		::compute_proof_multi(&ret.g1, &poly->_poly, (fr_t*) &x0[0], x0.size(), &settings);
		return ret;
	}
	bool check_proof_single(G1 commitment, G1 proof, Fr x, Fr y) {
		bool ret;
		::check_proof_single(&ret, &commitment.g1, &proof.g1, &x.fr, &y.fr, &settings);
		return ret;
	}
	bool check_proof_multi(G1 commitment, G1 proof, Fr x, std::vector<Fr> ys) {
		bool ret;
		::check_proof_multi(&ret, &commitment.g1, &proof.g1, &x.fr, (fr_t*)&ys[0], ys.size(), &settings);
		return ret;
	}
};